...
export const button: Buttons = {
	...
	execute: async (interaction) => {
		...
		const hiringDate: string = user.hiringDate ? dayjs(user.hiringDate).format('DD/MM/YYYY') || '' : '';
		...
		const lastMedicalVisit: string = user.last_medical_visit ? dayjs(user.last_medical_visit).format('DD/MM/YYYY') || '' : '';
		...
		const nextMedicalVisit: string = user.next_medical_visit ? dayjs(user.next_medical_visit).format('DD/MM/YYYY') || '' : '';
		...
		const lastMedicalVisitRow = new ActionRowBuilder().addComponents(lastMedicalVisitInput);
		const nextMedicalVisitRow = new ActionRowBuilder().addComponents(nextMedicalVisitInput);

		modal.addComponents(hiringRow, weaponRow, lastMedicalVisitRow, nextMedicalVisitRow);
