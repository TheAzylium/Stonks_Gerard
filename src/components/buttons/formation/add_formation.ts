...
export const button: Buttons = {
	...
	const roles = [FORMATEUR_ROLE_ID, HS_ROLE_ID, ADMIN_ROLE_ID];
		...
		const formationUser: FORMATION = await FormationSchema.findOne({ messageId: messageId }).lean();
		if (formationUser) {
			const completedFormationIds = formationUser.formations.map(f => f._id);

			const formationNotDoneByUser = Formation.filter(formation => !completedFormationIds.includes(formation._id));

			const options: {label: string; value: string}[] = formationNotDoneByUser.map(formation => ({ label: formation.name, value: formation._id }));
			...
			const selectFormationRow: any = new ActionRowBuilder().addComponents(selectFormation);
