...
export const command: SlashCommand = {
	...
	execute: async (interaction: CommandInteraction) => {
		try {
			...
			const targetUser: FORMATION = await FormationSchema.findOne({ discordId: target.id }).select('_id messageId').lean();
			if (targetUser) {
				...
				return await interaction.reply({
					content: 'Le membre est déjà dans la base de donnée :' + linkToMessage,
					ephemeral: true,
				})
			} else {
				...
				const message = await (interaction.guild.channels.cache.get(FORMATION_CHANNEL_ID) as TextChannel).send({
